...
diag_log "║░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░║";
diag_log "╠══════════════════════════════════════════════════╣";
diag_log format ["║ Start_Delay: %1",Start_Delay];

diag_log "║          << Initialisiere HC Link >>";
diag_log "╟──────────────────────────────────────────────────╢";
if((EQUAL(HC_ON,1))) then { [] execVM "\life_server\initHC.sqf"; };
...
if(isNil {GVAR_UINS "life_sql_id"}) then {
	...
	_result = EXTDB format["9:ADD_DATABASE_PROTOCOL:%1:sql_custom:%2:custom.ini",DATABASE_SELECTION,FETCH_CONST(life_sql_id)];
	if(!(EQUAL(_result,"[1]"))) exitWith {EXTDB_FAILED("extDB: Error with Database Connection")};
...
//[]call TON_fnc_cleanOldPlayer;
/*
master_group = createGroup civilian;
...
//Strip NPC's of weapons
{
	if(!isPlayer _x) then {
		_npc = _x;
		if!(_npc in ((units admin_team) + (units ad_base_1) + (units ad_base_2) + (units ad_base_3) + (units ad_base_4) + (units ad_base_5) + (units ad_base_6) + (units ad_base_7) + (units ad_base_8) + (units ad_base_9)  ))then {
			{
				if(!(EQUAL(_x,""))) then {
...
//[] execVM "\life_server\Scripte\Scheduler\Sched_Init.sqf";
...
_handle1 = [] spawn TON_fnc_initHouses; 
...
[] execVM "\life_server\Scripte\SperrZonen.sqf";
