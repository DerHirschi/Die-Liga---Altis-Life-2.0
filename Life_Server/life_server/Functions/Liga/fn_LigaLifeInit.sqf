...
// PlayerCount 
/*[[CIV´s],[COP´s],[LRK´s],[ALAC´s]]*/
//liga_plCount 		= [[],[],[],[]];
//liga_plCountNumb 	= [0,0,0,0];
...
[] call TON_fnc_LeimDBget; 
[] call AoD_fnc_marketconfiguration;
[] call TON_fnc_ligaWHinit;


/*
[]spawn{	
	while {true} do {	
	//	{if(isPlayer _x && alive _x)exitWith { admin_team setGroupOwner (owner _x); diag_log "gefunden";diag_log str _x;sleep 15;}; }forEach ([20277.3,8891.7,0.00156403] nearObjects ["Man", 150]) ;
		diag_log str (groupOwner admin_team);
		sleep 15;
		diag_log "next i";
	};
	diag_log "exit ";
