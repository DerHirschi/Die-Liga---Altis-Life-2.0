...
_query = format["vehicleUpdateActive:1:%1:%2:%3:%4:%5:%6:%7",_pid,_vid,_type,_repcount,_tuv,_trunk,_gear];
...
_vehicle SVAR["Trunk",_trunk,true];
...
if(EQUAL(SEL(_vInfo,1),"civ") && EQUAL(SEL(_vInfo,2),"B_Heli_Light_01_F") && !(EQUAL(SEL(_vInfo,8),13))) then {
	[_vehicle,"civ_littlebird",true] remoteExec ["life_fnc_vehicleAnimate",_unit];
};
