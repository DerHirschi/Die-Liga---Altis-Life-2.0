...
while {true} do
{
	...
	if((time - _timeend)    > 28630	)then {[]call TON_fnc_cleanup;[] call TON_fnc_RestartClean;[] call _fnc_dbSave;  _timeend = time;};	
	/*FPS LOG 1 Minute */
	if((time - _time1)     > 60		)then {[]call _fnc_srv_diag; _time1 = time;};
	...
	if((time - _time30)    > 1801	)then {/*[]call _fnc_fed;*/[]call _fnc_Dealer;[]call TON_fnc_cleanup;[] call _fnc_dbSave;if(EQUAL(LottoDay,3) )  then   {   []spawn TON_fnc_LottoZieh;  }; _time30 = time;};
	...
	if(SKY_HC_aktiv_1) then {
		{
			if(!isPlayer _x && alive _x) then {
				if!( (groupOwner (group _x)) isEqualTo HC_id ) then {
					(group _x) setGroupOwner HC_id;
					diag_log format["Gruppe an HC uebergeben : %1", (group _x)];
				};		
			};
		} foreach allUnits;
	};
