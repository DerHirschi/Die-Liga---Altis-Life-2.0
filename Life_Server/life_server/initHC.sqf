...
waitUntil { uiSleep 5; SKY_HC_aktiv_1};
	...
	diag_log "║ Uebergebe Mastergroup an HC ..... ";	
	 ...
	 admin_team setGroupOwner HC_id;
	 ad_base_1 setGroupOwner HC_id;
	 //ad_base_2 setGroupOwner HC_id;
	 
	 ad_base_4 setGroupOwner HC_id;
	 ad_base_5 setGroupOwner HC_id;
	 ad_base_6 setGroupOwner HC_id;
	 ad_base_7 setGroupOwner HC_id;
	 ad_base_8 setGroupOwner HC_id;
	 ad_base_9 setGroupOwner HC_id;
	 
	{
		_x addMPEventHandler ["MPHit",{[_this select 1] spawn npc_checkunit;}];
	} forEach ((units admin_team) + (units ad_base_1) + (units ad_base_3) + (units ad_base_4) + (units ad_base_5) + (units ad_base_6) + (units ad_base_7) + (units ad_base_8) + (units ad_base_9));

	
	[]spawn{
		sleep 130; 
		ad_base_3 setGroupOwner HC_id;
	};
