stig_sz_add =
	compileFinal "
	if(side player != west && ( call life_adminlevel <= 1) )exitWith{};
	_this spawn {
...
waitUntil {!isNull player && player == player && time > 1 && (!isNil "life_adminlevel")};
if(side player != west && ( call life_adminlevel <= 1) )exitWith{};
...
with uiNamespace do {	
	...
	waitUntil {private _adlevel = missionNamespace getVariable "life_adminlevel"; (!isNil "_adlevel")};
	waitUntil {!isNull player && player == player && time > 1 };
	private _adlevel = missionNamespace getVariable "life_adminlevel";
	
	if(side player != west && ( call _adlevel <= 1) )exitWith{};
