...
if(ad_debugRPT)then{	 
	diag_log "╠══════════════════════════════════════════════════╣";
	diag_log format["║ Craft Debug Rezept Array: %1",_rezept];
	diag_log "╠══════════════════════════════════════════════════╣";
	};
*/	
// Bye bye Tanoa
...
*/
	_rezept 		= M_CONFIG(getArray,"LigaProcess","CIVpro",_type);
	...
	_sleepp 		= 0.3;						// Standart Zeit
		...
		////// Zusatz Items  ///////
	if(_zu_item isEqualTo "") then {_var = 0;}else{
		_var = missionNamespace getVariable ([_zu_item,0] call life_fnc_varHandle);	
		if (_var != 0) then {
			switch (_zu_item) do
			{
	...
	if(EQUAL(_lic_check,1)) then {
		...
		else {_hasLicense = missionNamespace getVariable (([_type,0] call life_fnc_licenseType) select 0);};		//by Tonic
	...
	_i = 0;
		...
		if(_I_type isEqualTo 9) exitWith{	
...
////////////////////////////////////////  EXIT CHECKS ////////////////////////////////
	...
	if(_count_item isEqualTo 0) then {_exitItem = true;};  
	if(_count_Stk isEqualTo 0) then {_exitItem = true;};  	
	
	_count_item = _count_item - _count_Stk;
	
	if(_count_item != 0) then {_exitItem = true;};                  
...
if(_exitItem) exitWith { hint "Das Rezept hat ein Fehler .. bitte beim Admin melden .. Danke !!!";}; 	
	...
	{	
		...
		if(_x isEqualTo "a3") then{ _Val = liga_leim_tank; _tank_trig = true;} else {	
			_Val = missionNamespace getVariable ([_x,0] call life_fnc_varHandle);		// Menge aus Inventar auslesen
		};
		_ValChk = _stk_ar_weg select _i;																	// Menge aus Rezept Array auslesen
		...
		if (_multi isEqualTo 0) exitWith { _exitItem = true;};  													// Multiplikator = 0 dann raus ... 
		_i = _i + 1;																										// Index + 1
...
if(_exitItem) exitWith { hint "Du hast nicht genug Baumaterial dabei !!!";}; 	
...
/*	
if(ad_debugRPT)then{	 
	diag_log format["║ Craft Debug _count_Stk Array: %1",_count_Stk];
	diag_log format["║ Craft Debug _count_item Array: %1",_count_item ];
	diag_log format["║ Craft Debug _exitItem Array: %1",_exitItem ];
	diag_log format["║ Craft Debug _Inv_amoun_ar Array: %1",_Inv_amoun_ar ];	
	diag_log format["║ Craft Debug _multipli : %1",_multipli ];	
	diag_log format["║ Craft Debug _single : %1",_single ];	
	diag_log format["║ Craft Debug _itweight : %1",_itweight ];	
	diag_log format["║ Craft Debug _sleepp : %1",_sleepp ];	
	diag_log format["║ Craft Debug _zu_item : %1",_zu_item ];		
	diag_log "╠══════════════════════════════════════════════════╣";
};
*/
	////// Debug RPT ENDE	
if(_i != _count_Stk ) exitWith { hint"Fehler!! ... Bitte beim Admin melden... Danke fur deine Hilfe ...";
...
//////////////////// Dann fang mal an zu Bauen .... WARTEN ///////////////
	...
	if(_type != "moon" && _type != "methy") then {
		if(player distance _vendor > 10) exitWith {hint "Du musst in der naehe bleiben: 10m."; 5 cutText ["","PLAIN"]; [] execVM "statusBar.sqf";};
	};
	5 cutText ["","PLAIN"];
...
{		private ["_oldVal","_x","_oldVal_t"];		
		...
		_oldVal = _stk_ar_weg select _i_1;
...
if(_exitItem) exitWith { hint "Irgendwas laeuft mit den Gewichten deines Inventars schief !!!"; 5 cutText ["","PLAIN"];[] execVM "statusBar.sqf";};
