...
_maxGas = round (_maxGas * (missionNamespace getVariable ["mav_ttm_var_bohrturmMultiplier", 1]));
_maxOel = round (_maxOel * (missionNamespace getVariable ["mav_ttm_var_bohrturmMultiplier", 1]));
...
_gestangeTief =  (missionNamespace getVariable ["mav_ttm_var_bohrturmGestange", 100]);

/* Bohrgestänge nachlegen */
for [{_i=0},{_i<=5000 && (SEL(liga_bohrsteu,0))},{_i=_i+_gestangeTief}] do {// 50 * 100 Meter ( 50 Bohrgest MAX )
	...
	for [{_b=1},{_b<=_gestangeTief && (SEL(liga_bohrsteu,0))},{_b=_b+1}] do {
		...
		hint parseText format["
						...
						",_tiefe,((1 / _speedF) * 2),(_Gas * 40),(_Oel * 40),_itemval,_gestangeTief];
		
		uiSleep (_speedF / 2);
	};	
	["bohrturm_drill"] spawn mav_ttm_fnc_addExp;
	uiSleep (_speedF / 2);
};
