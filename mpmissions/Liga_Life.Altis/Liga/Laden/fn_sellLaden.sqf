...
if(_action) then {
	...
	life_pInact_curTarget SVAR ["ladextra",nil,true];

	life_pInact_curTarget SVAR["slots",nil,true];
	life_pInact_curTarget SVAR["locked",false,true]; 

	[life_pInact_curTarget] remoteExecCall ["TON_fnc_sellLaden",RSERV];	

	deleteMarker format["laden_%1",life_pInact_curTarget GVAR "luid"];
	deleteMarkerLocal format["laden_%1",life_pInact_curTarget GVAR "luid"];
	life_pInact_curTarget SVAR ["luid",nil,true];	
