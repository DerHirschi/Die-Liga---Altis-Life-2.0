...
// Trunk Abfrage KeyHandler  !!!! ÄNDERN 
#define KIND_TRUNK_KH(veh) \
	(isClass (missionConfigFile >> "LigaCfgKarren" >> ( typeOf veh ))) OR \
...
#define SVAR_PRONS uiNamespace setVariable
#define GVAR_PRONS uiNamespace getVariable
...
#define ITEM_VARNAME(varName) format["liga_inv_%1",M_CONFIG(getText,"VirtualLigaItems",varName,"variable")]
#define ITEM_VAR(var) (var splitString "_.") select 2
#define ITEM_VALUE(varName) GVAR_MNS [ITEM_VARNAME(varName),0]
#define ITEM_ILLEGAL(varName) M_CONFIG(getNumber,"VirtualLigaItems",varName,"illegal")
#define ITEM_SELLPRICE(varName) M_CONFIG(getNumber,"VirtualLigaItems",varName,"sellPrice")
#define ITEM_BUYPRICE(varName) M_CONFIG(getNumber,"VirtualLigaItems",varName,"buyPrice")
#define ITEM_NAME(varName) M_CONFIG(getText,"VirtualLigaItems",varName,"displayName")
...
#define ADCFG(ENT,CFG) (getNumber(missionConfigFile >> "Admin_config" >> ENT >> CFG  ))
