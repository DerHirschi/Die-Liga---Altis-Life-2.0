...
if!(EQUAL(typeOf _vehicle,"Land_fs_feed_F")) then {
	if(_vehicle isKindOf "House_F") then {
		private["_mWeight"];
		_mWeight = 0;
...
*/
if(_vehicle isKindOf "House_F" && {count (_vehicle GVAR ["containers",[]]) isEqualTo 0} && !(EQUAL(typeOf _vehicle,"Land_fs_feed_F"))) exitWith {
...
_vehicle spawn
{
	waitUntil {isNull (findDisplay 3500)};
	_this SVAR["trunk_in_use",false,true];
	if(EQUAL(typeOf _this,"Land_fs_feed_F")) exitWith {
		[_this] remoteExecCall ["TON_fnc_updateTankTrunk",RSERV];
	};
	if(isClass (missionConfigFile >> "LigaHousingClass" >> (typeOf _this))) then {		
