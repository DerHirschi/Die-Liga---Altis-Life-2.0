...
_Btn8 ctrlSetText "Entwaffnen";
_Btn8 buttonSetAction " [] remoteExec ['life_fnc_stripDownPlayer',life_pInact_curTarget];	        closeDialog 0;"; 
...
if( (0 call{ {	false; if((player distance2D _x) < 12 ) exitWith {true}}forEach Con_air_array })) exitWith {};
if(!((player distance (getMarkerPos "police_hq_1") < 30) OR  (player distance (getMarkerPos "cop_air") < 30) OR (player distance (getMarkerPos "Pol_Aut") < 30) OR (player distance (getMarkerPos "police_hq_3") < 30))  ) then 
{
