...
_fnc_food =  {
	...
	else
	{
		...
		if(life_hunger < 5) then {player setDamage 1; hint localize "STR_NOTF_EatMSG_Death";};
...
/*>>>>>>>> MAIN LOOP <<<<<<<<<<*/
	while{true} do
	{
		...
		UISL(0.3);	
		if(SIDECHECK					) then 	{if (SIDECHECK2) then {[player,false,playerSide] remoteExecCall ["TON_fnc_managesc",RSERV];[player,true,playerSide] remoteExecCall ["TON_fnc_managesc",RSERV];};};
		...
		if(SUBT(_DelGroupTime)	> 301	) then 	{15 setFog 0; 20 setOvercast (D41_Vorschau);setWind (D41_Wind);	60 setWaves (D41_Wellen);  _DelGroupTime = ZEIT;};
		...
		if(!(EQUAL(damage player,_dam)))	then {[] call life_fnc_hudUpdate;	SVAR_PRONS ["liga_dam",damage player];	};	
