...
if(([true,_type,_amount] call life_fnc_handleInv)) then{
	if(!isNil "_hideout" && {!isNil {grpPlayer getVariable "gang_bank"}} && {(grpPlayer getVariable "gang_bank") >= _price}) then {
		...
		if(_marketprice != -1) then  { 
			markt_type = _type;
			markt_amount = markt_amount + _amount;
			
			if(!(markt_type isEqualTo markt_type_1))then {
				markt_type_1 = markt_type;
				if(markt_trig)then{markt_trig = false;};
				[] spawn 
				{		
						sleep 0.6;
						markt_trig = true;
						_typ = markt_type;
						[] spawn { sleep 180; markt_trig = false;};
						waitUntil{sleep 14.3; !markt_trig};	
						if!(pvc_mp) then { [] spawn life_fnc_PVCMarketPrice;};
						sleep 5;									
						_amou = markt_amount;
						[_typ,_amou] call life_fnc_marketBuy;
