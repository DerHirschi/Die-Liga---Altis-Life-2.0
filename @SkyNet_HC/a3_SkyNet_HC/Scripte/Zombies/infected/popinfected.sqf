...
if!(Z_zoneakt)exitWith {diag_log "shit";};
diag_log "non_shit";
_mkr = _this;
_probability=10;
_exclusionList=[];
...
_houseList= nearestObjects [_pos ,["Land_GH_MainBuilding_right_F","Land_GH_MainBuilding_left_F","Land_GH_MainBuilding_middle_F","Land_GH_House_1_F","Land_GH_House_2_F","Land_Hotel_02_F","Land_House_Big_02_F","Land_House_Native_02_F","Land_House_Big_01_F","Land_House_Big_05_F","Land_House_Small_06_F","Land_House_Small_05_F","Land_i_Shed_Ind_F"],_distance];

_houseCount= count _houseList;

while {Z_zoneakt && ((count units infected) < 80)} do {
	{
		...
		if (!(typeOf _house in _exclusionList))then
		{
			for "_n" from 0 to 3  do {
				if (((count units infected) > 80) || (!Z_zoneakt)) exitWith{};
